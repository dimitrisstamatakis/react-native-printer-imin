...
public class PrinterIminModule extends ReactContextBaseJavaModule {
    ...
    public void getFontCodepage(final Promise promise) {
        try {
            if (iminPrintUtils == null) {
                List<String> fontCodepageList = PrinterHelper.getInstance().getFontCodepage();
                WritableArray fontCodepageArray = Arguments.createArray();
                for (String item : fontCodepageList) {
                    fontCodepageArray.pushString(item);
                }
                promise.resolve(fontCodepageArray);
            } else {
                promise.resolve(null);
            }
        } catch (Exception e) {
